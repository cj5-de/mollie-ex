defmodule MollieEx.CapturesTest do
  ...
  test "emits safe request telemetry for successful capture calls" do
    ...
    assert_success_telemetry(prefix, :captures_list, "GET", "/payments/{paymentId}/captures", 200)
  end
  ...
  defp capture_fixture_response(conn, status) do
    conn
    |> Plug.Conn.put_resp_header("content-type", "application/hal+json")
    |> Plug.Conn.send_resp(status, File.read!(@capture_fixture))
  end

  defp capture_list_fixture_response(conn, status) do
    conn
    |> Plug.Conn.put_resp_header("content-type", "application/hal+json")
    |> Plug.Conn.send_resp(status, File.read!(@capture_list_fixture))
  end
  ...
  defp assert_success_telemetry(prefix, operation, method, path_template, status) do
    ...
    refute telemetry_text =~ @api_key
    refute telemetry_text =~ "capture-123"
    refute telemetry_text =~ "Capture order #123"
    refute telemetry_text =~ "authorization"
  end
